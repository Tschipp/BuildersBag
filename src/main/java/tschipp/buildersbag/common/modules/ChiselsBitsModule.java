...
//
//			if (!(player.getItemInHand(hand).getItem() instanceof ItemChiseledBit))
//				return;
//
//			int placeStateID = place.usesBits() ? ItemChiseledBit.getStackState(player.getItemInHand(hand)) : 0;
//			int missingBits = 0;
//			int totalBits = 0;
//			int placedBits = 0;
//
...
//						} else if (!world.getBlockState(pos).getBlock().canBeReplaced(world, pos))
//							continue;
//
...
//						while (it.hasNext())
//						{
//							if (vb != null && ((vb.get(it.x(), it.y(), it.z()) != Block.getId(Blocks.AIR.defaultBlockState()) && !place.usesChisels()) || (vb.get(it.x(), it.y(), it.z()) == placeStateID && place.usesChisels())))
//							{
//								continue;
//							} else
//								bitCount++;
...
//
//			IContinuousInventory bits = new ContinousBits(acting, player.blockPosition(), placeStateID);
//
...
//
//				BlockState state = Block.stateById(placeStateID);
...
//
//								if (chisel.hurt(provided.size() * 16 * 16 * 16, new Random(), (ServerPlayerEntity) player))
//									chisel.shrink(1);
//
//								ItemStack providedBits = ItemChiseledBit.createStack(placeStateID, (provided.size() * 16 * 16 * 16) + (needsExtra ? 0 : placedBits - 64), true);
//								ItemEntity entityBits = new ItemEntity(world, player.getX(), player.getY(), player.getZ(), providedBits);
//
//
...
//						player.setItemInHand(hand, providedBits);
